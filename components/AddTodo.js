...
class AddTodo extends Component {  
    ...
    return (
      <View style={styles.container}>
        ...
        [blackColor,blueColor, greenColor, yellowColor, orangeColor, redColor].map((color,index) => {
          return (
            <TouchableHighlight key={color} style={{marginTop:10,marginLeft: 10,marginRight: 10,borderColor: 'gray',borderWidth: color === this.state.color ? 4:0}} onPress={()=>{
                this.setState({
                  color: color
                })
            }}>
  ...
  save() {
    ...
    dispatch(addTodo(this.state.text,this.state.color))
